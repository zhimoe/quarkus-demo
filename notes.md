...
`io.quarkus.runtime.StartupEvent`和`io.quarkus.runtime.ShutdownEvent`事件
...
Quarkus实现了JAX-RS规范, 支持@GET,@POST, @PUT等http动词注解.默认设置下, quarkus使用RESTEasy和Vertx框架, 而不是Servlet规范.如果需要使用Servlet, 则增加"
quarkus-undertow"扩展, 这是JBOSS（wildfly)服务器底层
通过@Path注解匹配URI.
